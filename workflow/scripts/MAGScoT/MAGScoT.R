...
if (is.null(opt$skip_merge_bins) == F | is.null(opt$score_only) == F | opt$n_iter == 0) {
  ...
  for (it in 1:opt$n_iter) {
    ...
    merge_cand <- gene_to_bin %>%
      distinct_at(.vars = c("bin", "gene"), .keep_all = T) %>%
      group_by(bin) %>%
      summarise(n_markers = n()) %>%
      left_join(gene_to_bin %>% group_by(bin) %>% summarise(n_markers_tot = n()), by = "bin") %>%
      filter(n_markers >= min_markers, n_markers_tot <= 150) %>%
      pull(bin) %>%
      unique()
    binmatch <- sapply(seq_along(merge_cand), function(thisbin_id) {
      ...
      thisbin_contigs <- contig_to_bin_bisco_it %>%
        filter(bin == thisbin, contig %in% gene_to_contig$contig) %>%
        pull(contig)
      thisbin_genes <- gene_to_contig %>% filter(contig %in% thisbin_contigs)
      if (it > 1) {
        thisbin_blacklist <- bisco_out %>%
          filter(bin == thisbin) %>%
          select(bin_a, bin_b) %>%
          unlist() %>%
          as.vector()
    ...
    bisco_out <- binmatch %>%
      do.call("rbind", .) %>%
      distinct_at(.vars = c("bin_a", "bin_b"), .keep_all = T) %>%
      filter(shared_markers >= min_markers) %>%
      mutate(bin = paste0(paste0("MAGScoT_", it, "_"), seq_along(shared_markers)))
    contig_to_bin_bisco_it <- bisco_out %>%
      apply(., 1, function(x) {
        contig_to_bin %>%
          filter(bin %in% c(x[2], x[3])) %>%
          mutate(bin = x[6], set = paste0("MAGScoT_", it)) %>%
          distinct_at(.vars = c("bin", "contig"), .keep_all = T)
      }) %>%
      do.call("rbind", .) %>%
      data.frame(row.names = NULL)
    contig_to_bin_bisco <- rbind(contig_to_bin_bisco, contig_to_bin_bisco_it)
    if (it == 1) {
      bisco_out_all <- bisco_out
    } else {
      bisco_out_all <- rbind(bisco_out_all, bisco_out)
    }
    if (nrow(contig_to_bin_bisco_it) == 0) {
      cat("Exiting bin merging, no (more) overlaps found\n")
      break
    }
    cat("\n")
  }
...
while (nrow(contig_to_bin.remain) > 1) {
  ...
  df_list <- sapply(unique(markers$set), function(this_set) {
    SET_MARKERS <- markers %>%
      ...
      pull(marker)
    zz %>%
      filter(gene %in% SET_MARKERS) %>%
      group_by(bin) %>%
      summarize(uniqueSCGs = n(), multipleSCGs = sum(count > 1), sumSCGs = sum(count)) %>%
      mutate(Completeness = uniqueSCGs / length(SET_MARKERS), additionalSCGs = sumSCGs - uniqueSCGs - multipleSCGs, Contamination = b * (multipleSCGs / uniqueSCGs) + c * (additionalSCGs / length(SET_MARKERS)), score = a * Completeness - Contamination)
  }, simplify = F)

  if (i == 1) {
    for (set in names(df_list)) {
      write.table(df_list[set], paste0(id, ".", set, ".out"), sep = "\t", row.names = F)
    }
  }
  ...
  scoreframe$max <- scoreframe %>%
    select(contains("score.")) %>%
    apply(., 1, max, na.rm = T)
  scoreframe$max_complete <- scoreframe %>%
    select(contains("Completeness.")) %>%
    apply(., 1, max, na.rm = T)

  scoreframe <- scoreframe %>%
    arrange(-max, set) %>%
    left_join(contig_to_bin %>% dplyr::select(bin, set) %>% distinct(), by = "bin")
  ...
  while (nrow(scoreframe) > 0 & scoreframe$set[1] == winnerset) {
    ...
    if (exists("scoreframe.out") == F) {
      scoreframe.out <- head(scoreframe, 1)
    } else {
      scoreframe.out <- rbind(scoreframe.out, head(scoreframe, 1))
    }
  ...
  recalc <- contig_to_bin.remain %>%
    filter(contig %in% contig_to_bin.out$contig) %>%
    pull(bin)
